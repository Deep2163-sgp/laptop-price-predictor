...
gpu = st.selectbox('Gpu_brand',df['Gpu_brand'].unique())
...
if st.button('Predict Price'):
    ...
    query = np.array([company,lap_type,ram,weight,touchscreen,ips,ppi,cpu,hdd,ssd,gpu,OpSys])
    query = query.reshape(1, 12)
    prediction = str(int(np.exp(pipe.predict(query)[0])))
    st.title("PREDICTED PRICE: " + prediction)
